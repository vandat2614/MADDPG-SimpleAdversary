...
class Agent:
    ...
    def choose_action(self, observation): # nhận vào 1 np array, trả về 1 numpy array (5, )
        state = T.tensor([observation], dtype=T.float).to(self.actor.device) # tới device của actor []
    ...
    #! trong action này có cộng thêm noise -> phân phối xác suất ko còn = 1??????????
